...
#include<vector>
#endif
// Used for the pre define custom color Library
// And it takes input in range of 0-255 and returns
// the output in range of 0.0000-1.00000 
std::vector<double> rgb(int r, int g, int b){
    std::vector<double> f;
...
// Used for the pre define custom color Library
// And it takes input in range of 0-255 and returns the invert 
// output in range of 0.0000-1.00000 
std::vector<double> rgb_invert(int r, int g, int b){
    std::vector<double> f;
    f.push_back(1-(double)r/255.0);
    f.push_back(1-(double)g/255.0);
    f.push_back(1-(double)b/255.0);
    return f;
}
std::vector<double> rgb_brightness(std::vector<double> color,double brigntness_in_percentage){
    color[0]*=brigntness_in_percentage/100;
    color[1]*=brigntness_in_percentage/100;
    color[2]*=brigntness_in_percentage/100;
    return color;
}
#endif